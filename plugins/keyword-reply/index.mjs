...
let fileRules = [];    // [{ keyword, folder }] 群文件整理规则，无内置关键词由用户自定

function scfg(p) {
  try {
    const d = path.dirname(p); if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true });
    fs.writeFileSync(p, JSON.stringify({ rules, admins, allowedGroups, fileRules }, null, 2), 'utf-8');
    return true;
...
function lcfg(p) {
  try {
    if (fs.existsSync(p)) {
      ...
      if (s.fileRules && Array.isArray(s.fileRules)) fileRules = s.fileRules;
    } else scfg(p);
...
const plugin_init = async (ctx) => {
  ...
  // API: 批量发送群公告
  ctx.router.postNoAuth("/send_notice", async (req, res) => {
    try {
      const b = typeof req.body === 'string' ? JSON.parse(req.body) : (req.body || {});
      const { group_ids, content, pinned, confirm_required, is_show_edit_card, tip_window_type, image } = b;

      if (!group_ids || !Array.isArray(group_ids) || group_ids.length === 0) {
        return res.json({ code: -1, message: "请选择至少一个群" });
      }
      if (!content || !content.trim()) {
        return res.json({ code: -1, message: "公告内容不能为空" });
      }

      const results = [];
      for (const raw of group_ids) {
        const gid = String(raw);
        if (!/^\d+$/.test(gid)) { results.push({ group_id: gid, status: "error", message: "无效群号" }); continue; }
        try {
          const params = {
            group_id: gid,
            ...
            tip_window_type: tip_window_type ?? 0,
          };
          if (image && typeof image === 'string') params.image = image;
          await ctx.actions.call("_send_group_notice", params, ctx.adapterName, ctx.pluginManager.config);
          results.push({ group_id: gid, status: "ok" });
        } catch (e) {
          log?.error(`[${PN}] 发送公告到群 ${gid} 失败:`, e);
          results.push({ group_id: gid, status: "error", message: e.message || String(e) });
        }
      }

      const ok = results.filter(r => r.status === "ok").length;
      const fail = results.filter(r => r.status === "error").length;
      res.json({ code: 0, data: { results, ok, fail, total: results.length } });
    } catch (e) {
  ...
  // API: 获取文件整理规则
  ctx.router.getNoAuth("/file_rules", (_req, res) => {
    res.json({ code: 0, data: fileRules });
  });

  // API: 保存文件整理规则
  ctx.router.postNoAuth("/file_rules", (req, res) => {
    try {
      const b = typeof req.body === 'string' ? JSON.parse(req.body) : (req.body || {});
      if (Array.isArray(b)) {
        fileRules = b.filter(r => r.keyword && r.keyword.trim() && r.folder && r.folder.trim())
          .map(r => ({ keyword: r.keyword.trim(), folder: r.folder.trim() }));
      }
      const ok = scfg(ctx.configPath);
      res.json({ code: ok ? 0 : -1, message: ok ? "保存成功" : "写入失败" });
    } catch (e) { res.json({ code: -1, message: e.message }); }
  ...
  // API: 整理群文件
  ctx.router.postNoAuth("/organize_files", async (req, res) => {
    try {
      const b = typeof req.body === 'string' ? JSON.parse(req.body) : (req.body || {});
      const { group_id } = b;
      if (!group_id) return res.json({ code: -1, message: "请指定群号" });
      const gid = String(group_id);

      if (!fileRules || fileRules.length === 0) {
        return res.json({ code: -1, message: "请先设置文件整理规则（关键词→文件夹）" });
      }

      const results = { scanned: 0, moved: 0, skipped: 0, errors: [], details: [] };

      // 1. 获取根目录文件列表
      let rootData;
      try {
        rootData = await ctx.actions.call("get_group_root_files", { group_id: gid, file_count: 999 }, ctx.adapterName, ctx.pluginManager.config);
      } catch (e) {
        return res.json({ code: -1, message: `获取群文件列表失败: ${e.message || e}` });
      }

      const rootFiles = rootData?.files || [];
      const rootFolders = rootData?.folders || [];

      // 2. 收集所有已存在的文件夹名称 → folder_id 映射（NTQQ 原始 folderId 格式）
      const folderMap = {};
      for (const f of rootFolders) {
        const name = f.folder_name || "";
        const id = f.folder_id || "";
        if (name && id) folderMap[name] = id;
      }

      // 3. 辅助函数：确保目标文件夹存在，返回用于 move 操作的目录路径
      //    QQ OIDB 协议要求目录格式为 "/" + folderId（根目录为 "/"）
      async function ensureFolder(folderName) {
        if (folderMap[folderName]) return folderMap[folderName];

        // 尝试创建文件夹
        try {
          await ctx.actions.call("create_group_file_folder", { group_id: gid, folder_name: folderName }, ctx.adapterName, ctx.pluginManager.config);
        } catch (e) {
          const msg = (e.message || e).toLowerCase();
          // 如果错误是"已存在"则继续，否则真实错误直接返回
          if (!msg.includes("exist") && !msg.includes("already") && !msg.includes("重复") && !msg.includes("已存在")) {
            log?.error(`[${PN}] 创建文件夹 "${folderName}" 失败:`, e);
            return "";
          }
        }

        // 轮询等待文件夹出现在根目录列表中（最多 5 次，间隔 1 秒）
        for (let attempt = 0; attempt < 5; attempt++) {
          if (attempt > 0) await new Promise(r => setTimeout(r, 1000));
          try {
            const fresh = await ctx.actions.call("get_group_root_files", { group_id: gid, file_count: 999 }, ctx.adapterName, ctx.pluginManager.config);
            for (const f of (fresh?.folders || [])) {
              const n = f.folder_name || "";
              const id = f.folder_id || "";
              if (n === folderName && id) {
                folderMap[folderName] = id;
                results.details.push({ file: `[创建文件夹]`, keyword: "", folder: folderName, status: "ensured" });
                return id;
              }
            }
          } catch (e2) {
            log?.warn(`[${PN}] 刷新文件列表失败(尝试 ${attempt + 1}): ${e2.message || e2}`);
          }
        }
        return ""; // 5 次均失败
      }
      ...
      // 4. 处理单个文件
      async function processFile(file, parentDir) {
        ...
        // 查找匹配的规则（按规则顺序，首次匹配，不区分大小写）
        const fileNameLower = fileName.toLowerCase();
        let matched = null;
        for (const fr of fileRules) {
          if (fileNameLower.includes(fr.keyword.toLowerCase())) { matched = fr; break; }
        }
        if (!matched) { results.skipped++; return; }

        // 确保目标文件夹存在
        const targetFolderId = await ensureFolder(matched.folder);
        if (!targetFolderId) {
          results.errors.push({ file: fileName, reason: `无法创建/获取文件夹 "${matched.folder}"` });
          return;
        }

        // 如果文件已在目标文件夹中，跳过
        if (parentDir === targetFolderId) { results.skipped++; return; }
        ...
        try {
          ...
          log?.info(`[${PN}] 移动文件: ${fileName} -> ${matched.folder} (parent=${parentDir}, target=${targetFolderId})`);
          await ctx.actions.call("move_group_file", params, ctx.adapterName, ctx.pluginManager.config);
          results.moved++;
          results.details.push({ file: fileName, keyword: matched.keyword, folder: matched.folder, status: "ok" });
        } catch (e) {
          results.errors.push({ file: fileName, reason: `移动失败: ${e.message || e}` });
        }
      }
      ...
      // 6. 递归处理子文件夹中的文件
      async function processSubFolders(folders) {
        for (const folder of folders) {
          const folderId = folder.folder_id || folder.FolderId || "";
          const folderName = folder.folder_name || folder.FolderName || "";
          if (!folderId) continue;
          // 记录文件夹到映射表（可能列表已包含）
          if (folderName && !folderMap[folderName]) folderMap[folderName] = folderId;
          try {
            const subData = await ctx.actions.call("get_group_files_by_folder", { group_id: gid, folder_id: folderId, file_count: 999 }, ctx.adapterName, ctx.pluginManager.config);
            for (const file of (subData?.files || [])) {
              await processFile(file, folderId);
            }
            if (subData?.folders?.length) {
              await processSubFolders(subData.folders);
            }
          } catch (e) {
            log?.warn(`[${PN}] 读取子文件夹 ${folderId}(${folderName}) 失败:`, e);
          }
        }
      }

      await processSubFolders(rootFolders);
